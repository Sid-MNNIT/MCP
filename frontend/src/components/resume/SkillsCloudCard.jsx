...
export default function SkillsCloudCard({ skills }) {
  return (
    <div className="resume-section">
      <div className="resume-section-header"><h3>Skills Cloud</h3><button className="btn-icon-action"><EditIcon /></button></div>
      <div className="resume-skills-wrapper">
        {skills.map((skill, i) => <span key={i} className="skill-pill">{skill}</span>)}
      </div>
    </div>
  );
}